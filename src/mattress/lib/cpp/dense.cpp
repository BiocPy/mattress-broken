...
inline uintptr_t initialize_dense_matrix_core(int nr, int nc, const T* ptr, bool byrow) { 
    ...
    if (byrow) {
        return reinterpret_cast<uintptr_t>(new tatami::DenseRowMatrix<double, int, decltype(view)>(nr, nc, view));
    } else {
        return reinterpret_cast<uintptr_t>(new tatami::DenseColumnMatrix<double, int, decltype(view)>(nr, nc, view));
    }
}
